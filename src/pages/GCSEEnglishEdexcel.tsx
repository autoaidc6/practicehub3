...
const TOPICS: Topic[] = [
  { 
    name: "English Literature: Macbeth", 
    grade: "8+", 
    videoUrl: "#",
    questionsUrl: "#",
    solutionsUrl: "#"
...
export default function GCSEEnglishEdexcel({ onBack }: { onBack: () => void }) {
    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-[#3498db] text-white p-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold tracking-wider uppercase">GCSE English (Edexcel)</h1>
        </div>
      </nav>

      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-black text-[#3498db] mb-4 tracking-tight uppercase">GCSE English Revision</h2>
          <p className="text-gray-600 max-w-2xl mx-auto font-medium">Comprehensive English revision resources for Edexcel.</p>
        </div>
              ...
              {grades.map(grade => (
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-bold transition-all ${
                    selectedGrades.includes(grade) ? 'bg-[#3498db] text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
                >
                  {grade}
                </button>
